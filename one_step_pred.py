# -*- coding: utf-8 -*-
"""
Created on Mon Jul  1 15:28:59 2019
...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  1 11:29:06 2019
...
mydf = mydf[-6*720:]
indexedDataset = mydf[:4119].copy()
indexedDataset = indexedDataset.set_index('timelist')

set_test = mydf[3559:3619].copy()
set_test = set_test.set_index('timelist')

dataset_test = set_test.values
...
dataset_total = training_set.copy()
...
moving_test_window = sc.transform(moving_test_window)
...
plt.plot(ans, color = 'blue', label = 'Predicted Google Stock Price')
...
"""
yesterday = today - timedelta(days=252)
con = psycopg2.connect(database="SLDCREP", user="postgres", password="createdon16082018", host="192.168.168.125", port="5432")
print("Database opened successfully")  
cur = con.cursor()  
...
an2df = yesdf[270:330]
an2df.timelist = pd.to_datetime(an2df.timelist,format='%Y-%m-%d %H:%M')
an2df = an2df.set_index('timelist')
"""
plt.figure(figsize=(20,10))
plt.plot(realdf, color = 'red', label = 'Real')
plt.plot(pdf, color = 'blue', label = 'Predicted')
plt.plot(an2df,color = 'green', label = 'Previous')
plt.xlabel('Time')
plt.ylabel('data')
plt.legend()
plt.show()
...
pldf = mydf.copy()
pldf = pldf[-3*720:]
pldf = pldf.set_index('timelist')
plt.plot(pldf.values)