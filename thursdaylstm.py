# -*- coding: utf-8 -*-
"""
Created on Tue Jun 25 18:26:19 2019
...
tlist = pardf['timelist']
pdf = pd.DataFrame()
pdf['timelist'] = pd.Series.tolist(tlist)
pdf['value'] = plist
pdf.timelist = pd.to_datetime(pdf.timelist,format = '%Y-%m-%d %H:%M')
pdf = pdf.set_index('timelist')
realdf.time = pd.to_datetime(realdf.time,format='%Y-%m-%d %H:%M')
realdf = realdf.set_index('time')
pardf.timelist = pd.to_datetime(pardf.timelist,format='%Y-%m-%d %H:%M')
...
pl = pd.DataFrame()
pl['values'] = plist
mlist = pd.Series.tolist(realdf.iloc[494:554,1])
pl['time'] = mlist
pl =pl.set_index('time')
plt.plot(realdf)
plt.plot(pl)
ax = realdf.plot()
pl.plot(ax=ax)